...
void Audio::setFileName(const char *pFileName)
{
...
bool Audio::load(void)
{
	...
	mChunk = Mix_LoadWAV(mFileName);
	if(mChunk == NULL)
		Console::log("[audio] Loading %s failed: %s", mFileName, Mix_GetError());
